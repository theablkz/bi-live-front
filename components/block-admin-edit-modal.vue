<template>
  <div class="block-admin-create-modal">
    <div>
      ...
      <button class="delete-button" @click="() => $emit('deleteBlock', editData.id)">Delete</button>

      <form @submit.prevent="createBlock">
        <label><small>Заголовок</small><input required v-model="editData.title" type="text"></label>
        <label><small>город</small>
          <select required v-model="editData.city">
        ...
        <label><small>формат</small>
          <select required v-model="editData.format">
            <option value="МАСТЕР-КЛАСС">МАСТЕР-КЛАСС</option>
            <option value="ВЕБИНАР">ВЕБИНАР</option>
          </select>
        </label>
        <label><small>Описание</small><input required v-model="editData.description" type="text"></label>
        <label><small>Дата проведения</small>
          <input  v-model="dateCalendar" type="date">
          <input  v-model="dateTime" type="time">
        </label>
        <label><small>Ссылка на ивент</small><input required v-model="editData.link" type="text"></label>
        <label><small>Изображение</small><input @input="uploadImage" type="file" accept="image/png, image/jpeg"></label>
        <label><small>Ссылка на видео</small><input required v-model="editData.videolink" type="text"></label>
        <label class="checkbox"><small>статус</small><input v-model="editData.status" type="checkbox"></label>
        <input type="submit">
    ...
    <div class="event-card">
      <div class="event-img-div">
        <img
          :src="`${baseUrl}/get-image/${editData.image}`"
      ...
      <div class="event-txt-div">
        <div class="event-label-div">
          <div class="event-label bestseller">{{editData.city}}</div>
          <div class="event-label">{{editData.format}}</div>
        </div>
        <div class="event-title">{{editData.title}}</div>
        <div>
        ...
        <div class="event-cta-div">
          <a :href="editData.link" target="_blank" class="btn-cta link-hover w-inline-block"
          ><div>Записаться</div>
            <div class="btn-border-full"></div></a
          ><a
          :href="editData.videolink"
          target="_blank"
...
export default {
  name: "block-admin-edit-modal",
  props: ['inputImage', 'editData'],
  data: () => ({
    title: '',
    city: '',
    status: true,
  ...
  computed: {
    ...
    dateJoiner() {
      return this.dateCalendar && this.dateTime ? new Date(`${this.dateCalendar} ${this.dateTime}`) : new Date(this.editData.date)
    }
  },
  methods: {
    async uploadImage(event){
      try {
        this.editData.image = await this.inputImage(event)
    ...
    createBlock(){
      const data = {
        ...
        format: this.editData.format,
        description: this.editData.description,
        date: this.dateCalendar && this.dateTime ? new Date(`${this.dateCalendar} ${this.dateTime}`) : this.editData.date,
        link: this.editData.link,
        image: this.editData.image,
        videolink: this.editData.videolink,
        categories: this.editData.categories
      }
      console.log(data)
      this.$emit('sendEditBlock', data)
    }
  }
}
</script>
